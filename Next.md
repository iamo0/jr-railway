...
## API роуты
- В Next.js существует возможность создавать обработчики GET и POST запросов, которые работают не только с разметкой, но и JSON данными (для создания API)

- Поскольку в Next.js роутинг построен на создании правильной структуры папок, для того, чтобы создать соответствующий роут нам нужно создать в соответствующей папке соответствующий файл

- Эти файлы называются API Route и именуются `route.ts`

- Файл должен возвращать функции `GET` и `POST` для обработки соответствующих запросов
